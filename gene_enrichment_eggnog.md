...
get eggnog genes into one file:
```
awk 'BEGIN{FS=OFS="\t"} !/^#/ {print FILENAME, $0}' *.emapper.annotations | sed 's/_eggnog.emapper.annotations//g' > all_eggnog.emapper.annotations
```
...
Step 2: Link to eggnog Annotations

```python
eggnog_annotations = pd.read_csv("all_eggnog.emapper.annotations", sep='\t',
                              names=['sample', 'gene_id', 'seed_ortholog', 'evalue', 'score', 'eggNOG_OGs', 'max_annot_lvl', 'COG_category', 'Description', 'Preferred_name', 'GOs', 'EC', 'KEGG_ko', 'KEGG_Pathway', 'KEGG_Module', 'KEGG_Reaction', 'KEGG_rclass', 'BRITE', 'KEGG_TC', 'CAZy', 'BiGG_Reaction', 'PFAMs'])

merged_data = pd.merge(eggnog_annotations, gene_counts, on=['gene_id', 'sample'])
```
...
```python
# group by amr_gene
eggnog_sample_grouped = merged_data.groupby(['KEGG_ko', 'sample'], as_index=False)[['mapped_reads', 'gene_length']].sum()

```

Step 4: Calculate RPK 
```python
eggnog_sample_grouped['RPK_gene'] = eggnog_sample_grouped['mapped_reads'] / (eggnog_sample_grouped['gene_length']/1000)
```
Step 5: Calculate Scaling Factor 
```python
scaling_factor = eggnog_sample_grouped.groupby(['sample'], as_index=False)['RPK_gene'].sum()
...
```python
tpm = pd.merge(eggnog_sample_grouped, scaling_factor, on='sample')
tpm['tpm'] = tpm['RPK_gene'] / tpm['scaling_factor']

# Save
tpm.to_csv("eggnog_KEGG_ko_tpm_within_samples.csv", index=False)
